...
6. If the user says "download it", use the CLI: `sds download FILE_ID`
...
|---------|-------------|---------|
| `sds search "query" FOLDER` | Search indexed images by description | `sds search "sunset" 0` |
| `sds index FOLDER` | Index all media in a Drive folder | `sds index https://drive.google.com/drive/folders/abc` |
| `sds browse FOLDER` | List files in a folder without indexing | `sds browse 0 --type images` |
| `sds list-folders` | Show all indexed folders with SDS IDs | `sds list-folders` |
| `sds download FILE_ID` | Download a file to the local filesystem | `sds download 1abc... -o photo.jpg` |
| `sds get-url FILE_ID` | Get a Drive viewing URL for a file | `sds get-url 1abc...` |

**Folder shorthand:** All commands that accept a folder accept three formats:
- **SDS ID** (integer from `sds list-folders`) — e.g. `0`, `1`, `2`
- **Drive folder ID** — e.g. `15kjKx9q...`
- **Full Drive URL** — e.g. `https://drive.google.com/drive/folders/15kjKx9q...`

SDS IDs are the easiest way to reference folders. Run `sds list-folders` to see the mapping.
...
- `sds download` accepts `--output / -o PATH` (defaults to current directory with the file's Drive name)
- All commands accept SDS IDs, folder URLs, or raw Drive IDs
